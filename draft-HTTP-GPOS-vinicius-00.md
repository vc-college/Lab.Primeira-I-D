**Working Group Name**  **Vinicius. Cruz**
...
## 1. Introduction
O uso de GPOS[RFC 1712] (Geographic Location, ou Localização Geográfica) em DNS é uma forma de enviar localização geográfica, ou seja, latitude, longitude e altitude do servidor. A ideia nessa ID é usar essa base para realizar conexões usando HTTP e enviar a localização. É importante ressaltar que esses dados de localização estarão dentro do servidor e farão parte da requisição HTTP.
...
O funcionamento desse novo formato de requisição deve ser de tal modo que:
  1. Uma requisição GET HTTP deve incluir um campo *GPOS: nome_dispositivo*.
  2. O servidor, em resposta à requisição deve responder no formato: nome_dispositivo ttl class *GPOS* longitude latitude altitude.

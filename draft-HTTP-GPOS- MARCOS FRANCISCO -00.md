...
## 1. Introduction
Na atualidade, com o avanço da tecnologia, uma função vem se tornando cada vez mais útil. Desde aplicativos como Waze e Google Maps, até jogos como Pokemon GO, a função
de geolocalização se mostra indispensável no mundo moderno.
O serviço de identificação do GPOS é implementado nan RFC1712 sobre DNS e é usado para conseguir a geolocalização do dispositivo servidor, aquele que o cliente está se
comunicando. Está implementação visa usar o GPOS sobre HTTP ao invés de DNS para adquirir a geolocalização do aparelho em questão.
